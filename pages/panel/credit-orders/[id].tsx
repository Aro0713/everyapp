...
export default function CreditOrderDetailsPage() {
  const router = useRouter();
  const id = typeof router.query.id === "string" ? router.query.id : "";

  const [row, setRow] = useState<CreditOrderRow | null>(null);
  ...
  async function load() {
    ...
    try {
      const r = await fetch(`/api/credit-orders/list?clientId=${encodeURIComponent(id)}`, {
        method: "GET",
        cache: "no-store",
      });
      ...
      const rows = Array.isArray(j?.rows) ? (j.rows as CreditOrderRow[]) : [];
      setRow(rows[0] ?? null);
    } catch (e: any) {
      setError(e?.message ?? "Nie udało się pobrać sprawy kredytowej.");
    } finally {
      setLoading(false);
    }
  }
  ...
  const readiness = useMemo(() => {
    ...
    if (
      row.credited_property_price !== null &&
      row.planned_own_contribution !== null &&
      row.loan_period_months !== null
    ) {
      return "Komplet podstawowy";
    }
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="mx-auto w-full max-w-[1600px] px-3 py-4 sm:px-4 lg:px-6">
        <div className="grid gap-4">
          <Card
            title="Sprawa kredytowa"
            subtitle="Szczegóły sprawy kredytowej klienta."
            actions={
              <>
                <button
                  type="button"
          ...
          >
            {loading ? (
              ...
              <div className="rounded-2xl border border-dashed border-white/15 bg-white/5 px-4 py-8 text-sm text-white/60">
                Nie znaleziono sprawy kredytowej.
              </div>
            ) : (
              <div className="grid gap-4">
                ...
                <div className="grid gap-4 xl:grid-cols-3">
                  <Card title="Finanse">
                    <div className="grid gap-3">
                      <Field
                        label="Cena kredytowanej nieruchomości"
                        value={fmtMoney(row.credited_property_price)}
                      />
                      <Field
                        label="Planowany wkład własny"
                        value={fmtMoney(row.planned_own_contribution)}
                      />
                      <Field
                        label="Okres kredytowania"
                        value={
                          row.loan_period_months != null
                            ? `${row.loan_period_months} mies.`
                            : "-"
                        }
                      />
                    </div>
                  ...
                  <Card title="Zakres sprawy">
                    <div className="grid gap-3">
                      <Field
                        label="Dotyczy posiadanej nieruchomości"
                        value={row.concerns_existing_property ? "Tak" : "Nie"}
                      />
                ...
                <Card
                  title="Dalsze kroki"
                  subtitle="Kontener roboczy pod kolejną rozbudowę procesu kredytowego."
                >
