...
function parseLocationPartsFromText(locationText: string | null): {
  ...
  const looksLikeVoiv =
    /^[a-zA-Ząćęłńóśźż-]{4,}$/.test(last) && !last.toLowerCase().startsWith("ul");
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const limit = Math.min(
      Math.max(Number(process.env.EVERYBOT_ENRICH_LIMIT ?? 150), 1),
      500
    );
          ...
          try {
            ...
            if (it.source === "otodom" && (!locationText || !locationText.trim())) {
              ...
              const pick = (s: string | null) => {
                if (!s) return null;
                const m = s.match(
                  /([A-Za-zĄĆĘŁŃÓŚŹŻąćęłńóśźż0-9 .-]+,\s*[A-Za-zĄĆĘŁŃÓŚŹŻąćęłńóśźż0-9 .-]+,\s*[A-Za-zĄĆĘŁŃÓŚŹŻąćęłńóśźż-]{4,})/
                );
    ...
    return res.status(200).json({ ok: true, processed, errors, limit });
  } catch (e: any) {
