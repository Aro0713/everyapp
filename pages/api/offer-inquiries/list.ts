  ...
  try {
    ...
    let where = `
      WHERE cc.office_id = $1
        AND cc.case_type = 'offer_inquiry'
    `;
    ...
    const sql = `
      SELECT
        ...
        cc.updated_at,
        p.full_name,
        coi.offer_id::text AS offer_id,
        coi.inquiry_text,
        coi.autofill_from_offer,
        coi.autofill_margin_percent
      FROM public.client_cases cc
      LEFT JOIN public.parties p
        ON p.id = cc.party_id
       AND p.office_id = cc.office_id
      LEFT JOIN public.client_case_offer_inquiries coi
        ON coi.client_case_id = cc.id
       AND coi.office_id = cc.office_id
      ${where}
      ORDER BY
    ...
    const { rows } = await pool.query<OfferInquiryRow>(sql, params);
    ...
    console.error("OFFER_INQUIRIES_LIST_ERROR", e);
