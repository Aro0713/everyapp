  ...
  try {
    ...
    const sql = `
      SELECT
        ...
        cc.updated_at,
        p.full_name,
        icd.insurance_subject::text AS insurance_subject,
        icd.insurance_notes
      FROM public.client_cases cc
      LEFT JOIN public.parties p
        ON p.id = cc.party_id
       AND p.office_id = cc.office_id
      LEFT JOIN public.client_case_insurance_details icd
        ON icd.client_case_id = cc.id
       AND icd.office_id = cc.office_id
      ${where}
      ORDER BY
    ...
    const { rows } = await pool.query<InsuranceOrderRow>(sql, params);
    ...
    console.error("INSURANCE_ORDERS_LIST_ERROR", e);
