// lib/everybot/enrichers/nieruchomosci-online.ts
import * as cheerio from "cheerio";
import type { Enricher, EnrichResult } from "./types";

/**
 * NIERUCHOMOSCI-ONLINE.PL – ENRICHER (detail page)
 * Portal w dużej mierze SSR/HTML. Da się wyciągnąć sensowne parametry z sekcji szczegółów.
 * Best-effort dla piętra/roku (często w opisie).
 */
...
function parseLocationParts(locationText?: string | null) {
  ...
  // często: "Miasto, Dzielnica" albo "Miasto (woj. ...)"
  const cleaned = locationText.replace(/\s*\(.*?\)\s*/g, "");
  ...
  return { voivodeship, city, district, street: null };
}
...
const noEnricher: Enricher = async (url: string): Promise<EnrichResult> => {
  ...
  out.owner_phone = null; // zwykle ukryty

  return out;
};

export default noEnricher;
