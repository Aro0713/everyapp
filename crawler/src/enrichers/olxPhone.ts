...
function looksLikeOlxListingUrl(sourceUrl: string): boolean {
  const url = sourceUrl.toLowerCase().trim();

  if (!url.includes("olx.pl")) return false;

  // pojedyncze ogłoszenie OLX zwykle kończy się IDxxxxxxxxx.html
  if (!/id[0-9a-z]+\.html(?:[?#].*)?$/i.test(url)) return false;
...
export async function revealOlxPhone(sourceUrl: string): Promise<OlxPhoneResult> {
  ...
  try {
    if (!looksLikeOlxListingUrl(sourceUrl)) {
      return {
        ok: false,
        owner_phone: null,
        ...
        debug: {
          ...
          pageTitle: null,
          error: "URL is not an OLX single listing",
        },
      };
    }
    ...
    if (clickedShowPhone) {
      await page.waitForFunction(() => {
        const texts = Array.from(
          document.querySelectorAll('[data-testid*="contact"], [data-testid*="phone"], [class*="contact"], [class*="phone"], aside')
        )
